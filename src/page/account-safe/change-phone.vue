  ...
  <div>
    ...
    <div class="container">
      ...
      <el-breadcrumb class="breadcrumb" separator-class="el-icon-arrow-right">
        <el-breadcrumb-item :to="{ path: '/account-safe' }" class="font-big">{{$t('changePhone.accountSafe')}}</el-breadcrumb-item>
        <el-breadcrumb-item class="font-big">{{$t('changePhone.changePhone')}}</el-breadcrumb-item>
      </el-breadcrumb>

      <!-- 更换绑定手机 -->
      <el-row class="form-box">
        <el-col :span="24" class="from-head">
          <span class="head-title">{{$t('changePhone.changePhone')}}</span>
          <i class="head-tips font-small iconfont icon-tishifill"></i>
          <span class="head-tips font-small">{{$t('changePhone.phoneInstruction')}}</span>
        </el-col>
        <el-col :span="12" :offset="6">
          <el-form label-position="top" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="ruleForm">
            <el-form-item :label="$t('changePhone.oldPhone')" prop="oldPhone">
              <el-input type="text" v-model="ruleForm.oldPhone" clearable>
              </el-input>
            </el-form-item>
            <el-form-item :label="$t('changePhone.newPhone')" prop="newPhone">
              <el-input type="text" v-model="ruleForm.newPhone" clearable>
                <el-select class="areaCode" v-model="ruleForm.areaCode" slot="prepend" :placeholder="$t('changePhone.placeholder')">
                  <el-option
            ...
            <el-form-item :label="$t('changePhone.smsValidate')" prop="smsCode">
              <el-input type="text" v-model="ruleForm.smsCode" clearable>
                <el-button :disabled="disabledBtn" :loading="verificationCodeFlag" @click="sendValidate('ruleForm')" class="validate-btn" type="text" slot="append">
                  {{$t('changePhone.smsValidate')}}
                  <span v-show="disabledBtn" class="disabledBtn">({{timer}})</span>
                </el-button>
              </el-input>
            </el-form-item>
            <el-form-item>
              <el-button :loading="updateLoadingFlag" type="primary" @click="submitForm('ruleForm')" class="sub-btn">{{$t('changePhone.confirm')}}</el-button>
            </el-form-item>
          </el-form>
        </el-col>
      </el-row>
...
<script type="text/ecmascript-6">
  ...
  import {_apiVerificationPhoneNum, _apiSendSMSphone, _apiModifyPhone, _apiGetUserInfo} from 'api'
  ...
  export default {
    ...
    data () {
      const validateOldPhone = (rule, value, callback) => {
        if (testPhone(value)) {
          callback()
        } else {
          callback(new Error(this.$t('changePhone.phoneEmptyMessage')))
        }
      }
      const validatePhone = (rule, value, callback) => {
        if (testPhone(value)) {
          if (this.ruleForm.areaCode) {
            let req = {phoneNum: this.ruleForm.newPhone}
            ...
            this.timerFlag = false
            callback(new Error(this.$t('changePhone.areaEmptyMessage')))
          }
        } else {
          callback(new Error(this.$t('changePhone.phoneEmptyMessage')))
          this.timerFlag = false
      ...
      const validateCode = (rule, value, callback) => {
        if (value === '') {
          callback(new Error(this.$t('changePhone.validateEmptyMessage')))
        } else {
          callback()
        }
      }
