<template>
  <div>
    <div class="agent-withdrawals" ref="content_box">
      <div class="withdrawals-head">
        <span class="margin-l-r-10">客户号:</span>
        <el-input class="input" v-model="requestFromData.customerCode" placeholder="请输入客户号"></el-input>
        <span class="margin-l-r-10 margin-left-30">金额:</span>
        <el-input class="input" v-model="requestFromData.enchashmentValMin" placeholder="请输入金额最小值"></el-input>
        <span class="margin-l-r-10">-</span>
        <el-input class="input" v-model="requestFromData.enchashmentValMax" placeholder="请输入金额最大值"></el-input>
        <span class="margin-l-r-10 margin-left-30">日期:</span>
        <el-date-picker
          v-model="requestFromData.beginDate"
          type="date"
          placeholder="选择开始日期">
        </el-date-picker>
        <span class="margin-l-r-10">-</span>
        <el-date-picker
          v-model="requestFromData.endDate"
          type="date"
          placeholder="选择截止日期">
        </el-date-picker>
        <el-button class="margin-left-30" type="primary" @click="getWithdrawHistory">查询</el-button>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span v-if="scope.row.lockStatus===1">已锁定</span>
            <el-button type="text" v-if="scope.row.lockStatus===0" @click="editLockStatus(scope.row)">修改</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
      ...
      <el-dialog
        :title="'客户'+dialogFormData.customerCode+'提现交易状态修改'"
        :visible.sync="dialogVisible" class="dialog">
        <el-form :rules="rules" ref="dialogFormData" :model="dialogFormData" label-width="80px">
          <el-form-item label="交易数量" prop="enchashmentVal">
            <el-input v-model="dialogFormData.enchashmentVal"></el-input>
          </el-form-item>
          <el-form-item label="交易状态"  prop="status_">
            <el-select style="width: 100%" v-model="dialogFormData.status_" placeholder="请选择交易状态">
              ...
              <el-option label="客户申请提现" value="1"></el-option>
              <el-option label="等待代理商确认" value="2"></el-option>
              <el-option label="代理商已付款" value="3"></el-option>
...
<script type="text/ecmascript-6">
  import { _apiAgentWithdrawHistory, _apiAgentWithdrawHistoryUpdate } from 'api'
  ...
  export default {
    ...
    methods: {
      ...
      // 重置搜索条件
      resetForm () {
        this.requestFromData.enchashmentValMin = ''
        this.requestFromData.enchashmentValMax = ''
      ...
      formatterStatus (row) {
        ...
        if (row.withdrawalStatus === 1) {
          return '客户申请提现'
        }
        if (row.withdrawalStatus === 2) {
          return '等待代理商确认'
        }
        if (row.withdrawalStatus === 3) {
          return '代理商已付款'
        }
        if (row.withdrawalStatus === 4) {
          return '交易成功'
        }
      },
      ...
      editLockStatus (obj) {
        ...
        this.dialogFormData.status_ = obj.withdrawalStatus + ''
        this.dialogFormData.enchashmentVal = obj.enchashmentVal + ''
      ...
      postEdit () {
        ...
        _apiAgentWithdrawHistoryUpdate({
          ...
          enchashmentVal: this.dialogFormData.enchashmentVal
        }).then((res) => {
